...
Matrix<3, 3> MEKF::skew3(Vector3f const& v) {
    Matrix<3, 3> skew;
    skew(0, 0) = 0;
    skew(0, 1) = -v.z;
    skew(0, 2) = v.y;
...
Vector3f MEKF::projectToHorizontal(Vector3f const& accel, Vector3f const& mag) {
...
void MEKF::predict(Vector3f const& gyro_meas, float dt) {
    float const DEG2RAD = 3.141592653589793f / 180.0f;
...
void MEKF::update(Vector3f const& accel_in, Vector3f const& mag_in) {
