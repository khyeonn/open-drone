...
public:
    ...
    float const &operator()(int i, int j) const { return data[i][j]; }

    float &operator()(int i) { return data[i][0]; }
    float const &operator()(int i) const { return data[i][0]; }
    ...
    Matrix<ROWS, OTHER_COLS> operator*(Matrix<COLS, OTHER_COLS> const &other) const {
    ...
    Matrix<ROWS, COLS> operator+(Matrix<ROWS, COLS> const &other) const {
        Matrix<ROWS, COLS> result;
    ...
    // ------- Vector operations
    float dot(Matrix<ROWS, 1> const &other) const {
        float result = 0.0f;
        for (int i = 0; i < ROWS; i++) {
    ...
    Matrix<3, 1> cross(Matrix<3, 1> const &other) const {
        Matrix<3, 1> result;
