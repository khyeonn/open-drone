...
struct Quaternion {
    ...
    Quaternion operator+(Quaternion const& other) const { return {w + other.w, x + other.x, y + other.y, z + other.z}; }
    ...
    Vector3f rotate(Vector3f const& v) const {
...
// i should have put this in its own header but it's here now and im too lazy to
// fix it
struct EulerAngles {
    float yaw;
    float pitch;
...
inline Vector3f quat2euler(Quaternion const& q) {
    Vector3f euler;
...
inline EulerAngles rad2deg(Vector3f const& in) {
    EulerAngles euler;
    float const RAD2DEG = 180.0f / 3.141592653589f;
...
inline Quaternion mat2quat(Matrix<3, 3> const& R) {
    Quaternion q;
    float trace;
