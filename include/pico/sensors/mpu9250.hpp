...
public:
    ...
    std::array<float, 3> const& get_accel() const { return accel; }
    std::array<float, 3> const& get_gyro() const { return gyro; }
    std::array<float, 3> const& get_mag() const { return mag; }
...
private:
    ...
    std::array<float, 3> gyro_bias_raw  = {0.0f, 0.0f, 0.0f};
    ...
    static constexpr uint8_t WHO_AM_I    = 0x75; // MPU9250 should return 0x75
