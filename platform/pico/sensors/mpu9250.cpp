#include "pico/sensors/mpu9250.hpp"
...
bool MPU9250::reset() {
    ...
    if (i2c_write_blocking(I2C_PORT, MPU9250_ADDR, reset, 2, false) != 2)
        return false;
    sleep_ms(200);

    uint8_t wake[] = {PWR_MGMT_1, 0x00};
    if (i2c_write_blocking(I2C_PORT, MPU9250_ADDR, wake, 2, false) != 2)
        return false;
    sleep_ms(200);
...
bool MPU9250::configure() { return configure_accel_gyro() && configure_mag(); }

bool MPU9250::configure_accel_gyro() {
    // Set sample rate
    uint8_t sample_rate[] = {SMPLRT_DIV, SAMPLE_RATE_DIV};
    if (i2c_write_blocking(I2C_PORT, MPU9250_ADDR, sample_rate, 2, false) != 2)
        return false;

    // Set accel range
    uint8_t accel_config[] = {ACCEL_CONFIG, ACCEL_CONFIG_VALUE};
    if (i2c_write_blocking(I2C_PORT, MPU9250_ADDR, accel_config, 2, false) != 2)
        return false;

    // Set gyro range
    uint8_t gyro_config[] = {GYRO_CONFIG, GYRO_CONFIG_VALUE};
    if (i2c_write_blocking(I2C_PORT, MPU9250_ADDR, gyro_config, 2, false) != 2)
        return false;
...
bool MPU9250::configure_mag() {
    ...
    if (i2c_write_blocking(I2C_PORT, MPU9250_ADDR, bypass, 2, false) != 2)
        return false;

    // Reset magnetometer before continuing
    uint8_t mag_reset[] = {AK8963_CNTL2, 0x01};
    if (i2c_write_blocking(I2C_PORT, AK8963_ADDR, mag_reset, 2, false) != 2)
        return false;
    sleep_ms(20);

    // Enter fuse ROM access mode to read sensitivity adjustment values
    uint8_t mag_fuse_rom_mod[] = {AK8963_CNTL1, 0x0F};
    if (i2c_write_blocking(I2C_PORT, AK8963_ADDR, mag_fuse_rom_mod, 2, false) != 2)
        return false;
    sleep_ms(100);
    ...
    if (i2c_write_blocking(I2C_PORT, AK8963_ADDR, &asa_reg, 1, true) != 1)
        return false;
    if (i2c_read_blocking(I2C_PORT, AK8963_ADDR, asa_values, 3, false) != 3)
        return false;

    // Mag adjustment values according to datasheet:
    ...
    if (i2c_write_blocking(I2C_PORT, AK8963_ADDR, power_down, 2, false) != 2)
        return false;
    sleep_ms(100);

    // Set to continuous measurement mode 2: 100Hz, 16-bit output
    uint8_t mag_config[] = {AK8963_CNTL1, 0x16};
    if (i2c_write_blocking(I2C_PORT, AK8963_ADDR, mag_config, 2, false) != 2)
        return false;
    sleep_ms(100);
...
void MPU9250::convert_mag_raw() {
    float const MAG_LSB_TO_UT = 0.15f;
...
void MPU9250::calibrate() {
    ...
    accel_bias[2] = accel_sum[2] / samples -
                    1; // adjust for gravity and current orientation of sensor (assuming Z axis is positive up)
...
void MPU9250::push_gyro_biases() {
    ...
    for (int i = 0; i < 3; i++) {
        int16_t offset_lsb =
            static_cast<int16_t>(-gyro_bias_raw[i] / 4); // OFFSET_LSB = X_OFFS_USR * 4 / 2^FS_SEL. Nominal FS_SEL = 0
...
void MPU9250::calibrate_mag() {
    ...
    int const samples = 1500;
    for (int i = 0; i < samples; i++) {
        read_mag_raw();

        for (int j = 0; j < 3; j++) {
            if (mag_raw[j] > mag_max[j])
                mag_max[j] = mag_raw[j];
            if (mag_raw[j] < mag_min[j])
                mag_min[j] = mag_raw[j];
        }
